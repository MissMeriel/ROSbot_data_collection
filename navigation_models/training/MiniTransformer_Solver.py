...
from MiniTransformer import VisionTransformer
from MiniTransformer_DataLoader import get_loader
...
class Solver(object):
	def __init__(self, args):
		...
		print("Moments of output distribution:", dataset.get_outputs_distribution())
		print("Total samples:", dataset.get_total_samples())
		def worker_init_fn(worker_id):
			np.random.seed(np.random.get_state()[1][0] + worker_id)

		self.train_loader = DataLoader(dataset, batch_size=args.batch, shuffle=True, worker_init_fn=worker_init_fn)
