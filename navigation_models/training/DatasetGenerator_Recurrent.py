...
class MultiDirectoryDataSequence(data.Dataset):
    def __init__(self, root, image_size=(100,100), transform=None, robustification=False, noise_level=10, recurrence_length=4):
        """
        Args:
            root_dir (string): Directory with all the images.
            transform (callable, optional): Optional transform to be applied
    ...
    def robustify(self, y_steer, image, recurrent_vec):
        if random.random() > 0.5:
            # flip image
            image = torch.flip(image, (2,))
            y_steer = -y_steer
            recurrent_vec = -recurrent_vec
        if random.random() > 0.5:
            # blur
        ...
        return y_steer, image, recurrent_vec
    ...
    def __getitem__(self, idx):
        if idx in self.cache:
            if self.robustification:
                ...
                recurrent_vec = copy.deepcopy(sample["recurrent_vec"])
                y_steer, image, recurrent_vec = self.robustify(y_steer, image)
                return {"image name": image, "angular_speed_z": y_steer, "linear_speed_x": sample["linear_speed_x"], "recurrent_vec": recurrent_vec}
            else:
                return self.cache[idx]
        ...
        # get recurrent vector
        orig_recurrent_vec = torch.zeros((1, self.recurrence_length), dtype=torch.float32)
        for i in range(self.recurrence_length):
            try:
                orig_recurrent_vec[0][i] = df.loc[df_index-i-1, 'angular_speed_z'].item()
            except Exception as e:
                pass
                # print(e)
                # print(f"{df_index=}")
                # print(f"{orig_y_steer=:.3f}    \t{orig_recurrent_vec=}")

        y_steer = copy.deepcopy(orig_y_steer)
        recurrent_vec = torch.clone(orig_recurrent_vec)
        if self.robustification:
            image = copy.deepcopy(orig_image)
            y_steer, image, recurrent_vec = self.robustify(y_steer, image, recurrent_vec)
        else:
            t = Compose([ToTensor()])
            image = t(image).float()
        ...
        sample = {"image": img_name.name, "image name": image, "angular_speed_z": torch.FloatTensor([y_steer]), "linear_speed_x": torch.FloatTensor([y_throttle]), "recurrent_vec": recurrent_vec}
        orig_sample = {"image name": orig_image, "angular_speed_z": torch.FloatTensor([orig_y_steer]), "linear_speed_x": torch.FloatTensor([y_throttle]), "recurrent_vec": orig_recurrent_vec}
